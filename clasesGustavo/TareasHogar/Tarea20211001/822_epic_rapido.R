...
kscript         <- "822_epic_ajus_rapido_sinOBNov"

karch_dataset    <- "./datasets/dataset_epic_ajus_v004.csv.gz"   #este dataset se genero en el script 811_dataset_epic.r

kapply_mes       <- c(202011)  #El mes donde debo aplicar el modelo

ktest_mes_hasta  <- 202010  #Esto es lo que uso para testing
...
ktrain_meses_malos  <- c( 202006,201904 )  #meses que quiero excluir del entrenamiento


kgen_mes_hasta    <- 202010   #La generacion final para Kaggle, sin undersampling
...
hs <- makeParamSet( 
  makeNumericParam("learning_rate",    lower=    0.04 , upper=    0.04),
  makeNumericParam("feature_fraction", lower=    0.7148  , upper=    0.7148),
  makeIntegerParam("min_data_in_leaf", lower=  6334L   , upper= 6334L),
  makeIntegerParam("num_leaves",       lower=  731L   , upper= 731L)
)

campos_malos  <- c()   #aqui se deben cargar todos los campos culpables del Data Drifting

ksemilla_azar  <- 102193  #Aqui poner la propia semilla
#------------------------------------------------------------------------------
...
kimp          <- paste0("./work/E",  kexperimento, "/E",  kexperimento, "", kscript, "" )
kkaggle       <- paste0("./kaggle/E",kexperimento, "/E",  kexperimento, "", kscript, "" )
kkagglemeseta <- paste0("./kaggle/E",kexperimento, "/meseta/E",  kexperimento, "", kscript, "" )
