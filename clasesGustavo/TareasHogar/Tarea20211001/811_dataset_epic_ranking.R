...
AgregarVariables  <- function( dataset )
{
  ...
  for (i in unique(dataset$foto_mes)){
    dataset[foto_mes==i, paste0( campos_pesos, "_rank") := lapply(.SD,frankv ,ties.method = "dense",na.last = "keep"), .SD = campos_pesos]
    j=j+1
  }
  ...
  #setnames(dataset,paste0( campos_pesos, "_rank"),campos_pesos)
  ...
  dataset[ , ratio_quater            := cpayroll_trx  / ctrx_quarter ]
  dataset[ , ratio_visa              := mtarjeta_visa_consumo  / ctarjeta_visa_transacciones ]
  dataset[ , ratio_qtr_mv            := ctrx_quarter  / mv_msaldototal ]
  
  dataset[ , ratio_pay_mv            := cpayroll_trx  / mv_msaldototal ]
  dataset[ , ratio_pay_tarj          := cpayroll_trx  / mtarjeta_visa_consumo ]
  dataset[ , ratio_pay_caj           := cpayroll_trx  / mcaja_ahorro ]
